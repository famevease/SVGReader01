...
/*
* Class đại diện và xử lý các màu RGBA
* Đọc, lưu trữ và in màu của các hình vẽ
*/
class ColorShape {
public:
	ColorShape(); // Default constructor (0,0,0,255) -> màu đen 
	ColorShape(int red, int green, int blue, int alpha = 255);	// red: cường độ đỏ [0;255]
																// green: cường độ màu xanh lá cây [0;255]
																// blue: cường độ màu xanh dương [0;255]
																// Alpha: opacity độ trong suốt [0;255]

	explicit ColorShape(int color); // Contruct màu từ 32-bit unsigned integer 
									// 4 tham số mỗi tham số 8 bits => Tổng 32 bit = 1 int

	friend ostream& operator<<(ostream& os, const ColorShape& color);	// Tô màu
																		// os: ouput, color: màu được in

	static const ColorShape Black;    ///< Màu đen
	static const ColorShape White;    ///< Màu trắng
	static const ColorShape Red;      ///< Màu đỏ
	static const ColorShape Green;    ///< Màu xanh lá
	static const ColorShape Blue;     ///< Màu xanh dương
	static const ColorShape Yellow;   ///< Màu vàng
	static const ColorShape Magenta;  ///< Màu hông đỏ
	static const ColorShape Cyan;     ///< Màu xanh lơ
	static const ColorShape Transparent;  ///< Hoàn toàn trong suốt

	int r; // cường độ đỏ
	int g; // cường độ xanh lá
	int b; // cường độ xanh dương
	int a;	// độ trong suốt
...
/*
* Bảng màu
* Tạo map với key là tên màu bằng SFML 
*/
